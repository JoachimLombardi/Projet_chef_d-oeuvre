...
def create_affiliation(affiliation: schemas.AffiliationsCreate, db: Session = Depends(get_db)):
    db_affiliation = Affiliations(name=affiliation.name)
...
def create_authorship(authorship: schemas.AuthorshipCreate, db: Session = Depends(get_db)):
    db_authorship = Authorship(**authorship.model_dump())
...
def get_article(article_id: int, db: Session = Depends(get_db)):
    ...
    authors = db.query(Authors).join(Authorship).filter(Authorship.id_article == article_id).all()
