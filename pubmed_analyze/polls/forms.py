...
class ArticleForm(forms.ModelForm):
    class Meta:
        model = Article
        fields = ['title_review', 'pub_date', 'title', 'abstract', 'pmid', 'doi', 'disclosure', 'mesh_terms', 'url']
...
class AuthorForm(forms.Form):
    ...
    affiliations = forms.CharField(
        label="Affiliations (séparées par des virgules)", 
        widget=forms.Textarea(attrs={'class': 'form-control', 'rows': 3}),
        help_text="Entrez les affiliations séparées par des virgules"
    )
